...
```text
Logged in as: shaan@krypthq.com
Projects: my-app, billing-service, infra
```
...
```text
my-app            3 environments    2 members
billing-service   2 environments    1 member
infra             1 environment     4 members
```
...
Compares secrets between two environments of the same project. Shows which keys were added, removed, or changed.

### Usage

```bash
krypt diff --envA <environment> --envB <environment>
```
...
```text
+ STRIPE_LIVE_KEY     (only in production)
